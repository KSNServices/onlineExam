...
public class TeacherAdminLoginController {
	@Value("${emailFrom}")
	String emailFrom;
	...
	@RequestMapping("/teacherAdminLogin")
	public String showManageUsers(Map<String, Object> map,HttpSession session) {
		map.put("user", new User());
		map.put("userList", userService.listUserByRole(ModelConstants.TEACHER));
		map.put("paymentPlanList", paymentPlanService.listPaymentPlan());
		return "teacherAdminLogin";
	}

	@RequestMapping("/teacherUsers/{searchString}")
	public String searchUsers(Map<String, Object> map,HttpSession session, @PathVariable("searchString") String searchString) {
		if(null == searchString || searchString.length()<=0){
			return "redirect:/teacherUser/teacherAdminLogin";
		}
		map.put("user", new User(new User()));
		map.put("paymentPlanList", paymentPlanService.listPaymentPlan());
		map.put("userList", userService.listUser(((User)session.getAttribute("userDetails")).getAdminUser().getId(),searchString));
		return "teacherAdminLogin";
	}
	
	@RequestMapping(value = "/saveTeacherAdmin", method = RequestMethod.POST)
	public String saveUser(Map<String, Object> map,HttpSession session, @ModelAttribute("user") User user, @Valid User userValid,
			BindingResult result,@RequestParam("userpic") MultipartFile userpic) {
		if (result.hasErrors()) {
			...
			try {
				if(null == user.getId()){
					...
					user.setAuthority(ModelConstants.TEACHER);
	...
	public String deleteUser(Map<String, Object> map,HttpSession session, @PathVariable("userNo") Integer userNo) {
		...
		if (!user.getUserName().equals("admin")) {
			...
			return "redirect:/studentUser/studentAdminLogin";
		} else {
	...
	public String retrievePassword(Map<String, Object> map,HttpSession session, @PathVariable("userName") String userName) {
	...
	public String saveProfile(Map<String, Object> map, HttpSession session, @ModelAttribute("userdetails") User user,
		...
		if (result.hasErrors()) {
			...
			try {
				...
				return "redirect:/studentUser/profile";
			} catch (Exception exp) {
	...
	public String userNotification(Map<String, Object> map, HttpSession session,HttpServletRequest request)throws Exception {
		Integer userId = ((User) session.getAttribute("userDetails")).getId();
		User user = userService.getUserById(userId);
		map.put("userdetails", user);
		if (request.getParameter("message") != null) {
	...
	public String readUserImage(Map<String, Object> map, HttpSession session, HttpServletResponse httpServletResponse)throws Exception {
		...
		map.put("user",user);
		return "redirect:/teacherUser/profilePage";
	}
	...
	public String uploadManageCompanyLogo(Map<String, Object> map, @RequestParam("image") MultipartFile image,
		...
		try {
			if (!image.isEmpty()) {
				...
				String logoDirectory = ERPConstant.ERP_MEDIA_DIRECTORY + File.separator
						+ ERPConstant.PROFILE_IMAGES;
	...
	public void readUserDisplayImage(Map<String, Object> map, HttpSession session,@PathVariable("userId") Integer userId,
