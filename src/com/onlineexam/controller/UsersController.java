...
public class UsersController {
	...
	public String showManageUsers(Map<String, Object> map,HttpSession session) {
		...
		User u = new User();
		map.put("userList", userService.listUserByRole(ModelConstants.ADMIN));
		String adminUniqueId = userService.getLastId(ModelConstants.ADMIN);
		int adminID=0;
		if (adminUniqueId != null) {
			adminID = Integer.parseInt(adminUniqueId) + 1;
		}else{
			adminID=1;
		}
		u.setUserNumber(String.valueOf(adminID));
		...
		if (result.hasErrors()) {
			...
			try {
				if(null == user.getId()){
					...
					user.setAuthority(ModelConstants.ADMIN);
