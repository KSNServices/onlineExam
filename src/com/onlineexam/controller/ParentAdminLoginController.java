...
public class ParentAdminLoginController {
	@Value("${emailFrom}")
	String emailFrom;
	...
	@RequestMapping("/parentAdminLogin")
	public String showManageUsers(Map<String, Object> map,HttpSession session) {
		map.put("user", new User());
		map.put("userList", userService.listUserByRole(ModelConstants.PARENT));
		map.put("paymentPlanList", paymentPlanService.listPaymentPlan());
		return "parentAdminLogin";
	}

	@RequestMapping("/parentUsers/{searchString}")
	public String searchUsers(Map<String, Object> map,HttpSession session, @PathVariable("searchString") String searchString) {
		if(null == searchString || searchString.length()<=0){
			return "redirect:/parentUser/parentAdminLogin";
		}
		...
		if (result.hasErrors()) {
			...
			try {
				if(null == user.getId()){
					...
					user.setAuthority(ModelConstants.PARENT);
	...
	public String saveProfile(Map<String, Object> map, HttpSession session, @ModelAttribute("userdetails") User user,
		...
		if (result.hasErrors()) {
			...
			try {
				...
				return "redirect:/parentUser/profile";
			} catch (Exception exp) {
	...
	public String readUserImage(Map<String, Object> map, HttpSession session, HttpServletResponse httpServletResponse)throws Exception {
		...
		map.put("user",user);
		return "redirect:/parentUser/profilePage";
	}
